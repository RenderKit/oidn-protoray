...
public:
  Diffuse2RendererStream(const std::shared_ptr<const Scene>& scene, const std::shared_ptr<IntersectorStream<streamSize>>& intersector, const Props& props)
  ...
  void renderTile(const Vec2i& tileId, int tid)
  {
    ...
    for (;;)
    {
      ...
      for (int i = 0; i < missCount; i += simdSize)
      {
        ...
        if (accum)
          frameBuffer->getColor().add(m, pixel, Vec3vf(L));
        else
          frameBuffer->getColor().set(m, pixel, Vec3vf(L));
      }
