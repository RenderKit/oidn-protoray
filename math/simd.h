...
prt_inline void set(const var<bool,N>& mask, var<T,N>& a, const var<T,N>& b)
{
  a = select(mask, b, a);
}
...
// SIMD loop helpers
prt_inline int getSimdLoopEnd(int count)
{
  return count & (-simdSize);
}

prt_inline vbool getSimdLoopTailMask(int count)
{
...
/*
prt_inline vbool getSimdMaskFromCount(int count)
{
...
template <class T, int N>
prt_inline T nextUnique(var<bool,N>& mask, const var<T,N>& vi, var<bool,N>& mask_i)
{
...
template <int N>
prt_inline var<float,N> toFloat(const var<float,N>& x) { return x; }

template <int N>
prt_inline var<float,N> toFloatUnorm(const var<uint,N>& x)
{
  return toFloat(x) * 0x1.0p-32f;
}

template <int N>
prt_inline var<float,N> toFloatUnormExcl(const var<uint,N>& x)
{
...
template <int N>
prt_inline var<float,N> asFloat(const var<float,N>& x) { return x; }

template <int N>
prt_inline var<int,N> toInt(const var<int,N>& x) { return x; }

template <int N>
prt_inline var<int,N> asInt(const var<int,N>& x) { return x; }

template <int N, class T>
prt_inline var<T,N> toVar(T x) { return var<T,N>(x); }

template <int N>
prt_inline var<int,N> toIntSafe(const var<float,N>& x)
{
...
template <class T, int N>
prt_inline var<T,N> rcpSafe(const var<T,N>& x)
{
  return select(x == zero, posMax, rcp(x));
}

template <class T, int N>
prt_inline var<bool,N> isfinite(const var<T,N>& x)
{
  return (x >= var<T,N>(negMax)) & (x <= var<T,N>(posMax));
}

template <int N>
prt_inline var<float,N> signBit(const var<float,N>& x)
{
  return x & asFloat(var<int,N>(0x80000000));
}

template <int N>
prt_inline var<float,N> minInt(const var<float,N>& a, const var<float,N>& b)
{
  return asFloat(min(asInt(a), asInt(b)));
}

template <int N>
prt_inline var<float,N> maxInt(const var<float,N>& a, const var<float,N>& b)
{
  return asFloat(max(asInt(a), asInt(b)));
}

template <int N>
prt_inline var<uint,N> reverseBits(var<uint,N> x)
{
...
template <int N>
prt_inline var<float,N> sin(const var<float,N>& xFull)
{
...
template <int N>
prt_inline var<float,N> cos(const var<float,N>& xFull)
{
  static const float piOverTwoVec = 1.57079637050628662109375;
  static const float twoOverPiVec = 0.636619746685028076171875;
  auto scaled = xFull * twoOverPiVec;
...
template <int N>
prt_inline void sincos(const var<float,N>& xFull, var<float,N>& sinResult, var<float,N>& cosResult)
{
...
template <int N>
prt_inline var<float,N> tan(const var<float,N>& xFull)
{
...
template <int N>
prt_inline var<float,N> asin(const var<float,N>& x0)
{
  auto isneg = (x0 < 0.f);
  auto x = abs(x0);
  auto isnan = (x > 1.f);
...
template <int N>
prt_inline var<float,N> acos(const var<float,N>& v)
{
  return 1.57079637050628662109375 - asin(v);
}

template <int N>
prt_inline var<float,N> atan(const var<float,N>& xFull)
{
  const float piOverTwoVec = 1.57079637050628662109375;
...
template <int N>
prt_inline void __rangeReduceLog(const var<float,N>& input, var<float,N>& reduced, var<int,N>& exponent)
{
...
template <int N>
prt_inline var<float,N> log(const var<float,N>& xFull)
{
  var<float,N> reduced;
  var<int,N> exponent;
...
template <int N>
prt_inline var<float,N> pow(const var<float,N>& x, const var<float,N>& y)
{
  auto x1 = abs(x);
  auto z = exp(y * log(x1));
...
template <int N>
prt_inline var<float,N> pow(const var<float,N>& x, float y)
{
